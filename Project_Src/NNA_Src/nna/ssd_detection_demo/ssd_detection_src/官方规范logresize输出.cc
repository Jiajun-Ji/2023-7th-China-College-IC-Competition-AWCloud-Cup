...
ImageBlob prepare_imgdata(const cv::Mat& img,
  ...
  img_data.scale_factor_ = {
    static_cast<float>(target_size_[0]) / static_cast<float>(300),
    static_cast<float>(target_size_[1]) / static_cast<float>(300)
  };
...
//	*(ddr3_vga_cfg_base + 2) = 0x00000001;
  ...
  while(1)
  {
    ...
    {	
      //指向矩阵当前行的指针
      Vec<uchar, 1> *data_Ptr = rgb_img.ptr<Vec<uchar, 1>> (row);
      //可以将data_Ptr看出一个数组，data_Ptr[col]代表了第几个元素
      //每个元素的数据类型是<ushort,2>，包含了2个无符号整型的元素
    ...
    // 输出图像测试
    if(master_ctrl_virtual_base[3] == 0x1)
    {
