...
//	*(ddr3_vga_cfg_base + 2) = 0x00000001;
  ...
  while(1)
  {
    ...
    {
      // printf("read buffer0\n");
      memcpy(buf, transfer_data_base+ IMG_BUF_SIZE*5, IMG_BUF_SIZE);//vga写buffer3
      // ddr3_vga_top_virtual_base[1] = dma_base + IMG_BUF_SIZE * 3;//vga读buffer4
    }
    else//该值为0代表着dvp写入的是buffer1，则从buffer0中读
    {
      // printf("read buffer1\n");
      memcpy(buf, transfer_data_base +IMG_BUF_SIZE*4, IMG_BUF_SIZE);//vga写buffer4
    ...
    {	
      ...
      // memcpy(data_Ptr, buf+(row+1)*300, 300);
